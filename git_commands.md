...
1. `mv <file_name> <dir_name>/[<dir_name>/]`
    + 특정 파일을 `ls` 명령어에서 사용하는 모든 디렉터리 경로로 이동시킬 수 있다. "../"가 상위 디렉터리(폴더)라는 것을 기억하면 꽤 유용할 수 있다.
...
3. `git switch [-c] <branch_name>`
    + 기존 작업 브랜치(HEAD)에서 현재 존재하는 다른 브랜치로 이동할 때 사용한다. 중간에 -c 옵션을 추가해주면 새로운 이름의 브랜치를 생성하고 동시에 그 브랜치로 이동한다. 
    + 브랜치 생성 시에는 항상 기존에 있는 이름들과 중복되지 않게 하는 것이 중요하다.
...
2. `git add <file_name> [<file_name>]`
    ...
    + `git add` 하기 전에 `git status`를 했을 때 <i>Untracked files: </i> 목록에 새로 생성한 파일들이 <b><span style="color: red" >빨간색으로 표시</span></b>되어 있다면 그 파일들은 스테이지에 복사할 수 있는 상태가 된다. <i>Untracked</i> 상태는 <i><b>Git</b></i> 저장소에서 그 파일은 관여하지 않는다는 의미이다. 이건 앞으로 꽤 중요한 내용이니 기억해두기 바란다. 그 외에 `add` 명령어로 상태를 바꿀 수 있는 다른 상태로는 <i>New, Modified, Deleted</i> 등이 있다. 
...
1. `git log [-n<number>]`
    + `log` 명령은 `commit` 혹은 그와 직결된 대부분의 명령을 실행한 후에 커밋을 확인하기 위한 용도로 쓰인다. `-n` 옵션은 가장 최근에 등록된 number개의 커밋만 보여준다. 이 옵션이 없으면 최근-시작 순으로 전체 다 보여준다.
2. `git log [--oneline]`
    + `--oneline` 옵션은 보통 커밋 내용 전체를 다 보여주던 걸 한 줄로 간략하게 줄일 수 있게 한다. 최소한의 정보지만 브랜치, 그래프, 태그 등 필요한 정보만 확실하게 보고 싶을 때 사용한다.
3. `git log [--all]`
    + 보통 로컬 저장소에 여러 브랜치를 동시에 사용할 때 좋은 옵션 중 하나이다. 현재 작업중인 브랜치(HEAD) 뿐 아니라 디렉터리에 있는 모든 브랜치에 있는 커밋들을 보고 싶을 때 추가하면 된다.
4. `git log [--graph]`
    + `--graph` 옵션은 `--all` 옵션과 함께 여러 브랜치를 사용할 때 유용하게 쓰인다. 물론 다른 옵션들과 함께 쓸 수 있으며 중급 기능에 해당하는 `merge` 와 `rebase` 명령어를 사용할 때 가장 많이 쓰인다.
5. `git log [--decoration]`
    + 브랜치의 참조나 태그 등을 좀 더 간결하게 보고 싶을 때 사용하면 된다. 개인적으로는 잘 안 쓰는 옵션이다.
6. `git log`의 가장 많이 사용하는 옵션들 정리
    1. `git log -n5`
    2. `git log --oneline -n5`
    3. `git log --oneline --all -n5`
    4. `git log --oneline --all --graph -n5`

### <b><i>Git</i></b> 명령어의 별칭(Alias)을 지정하는 방법
> 명령어 한 줄이 너무 길 때 반복되는 명령문을 편하게 쓰기 위해 간단하게 별칭을 붙여줄 수 있다. 

1. `git config --global alias.<alias_name> "<command_statement>"`
    + `<alias_name>` 자리에 원하는 별칭을 넣고, `<command_statement>` 자리에 줄이고 싶은 명령문의 <u>`git`을 제외한 전체 내용</u>을 작성하면 새로운 별칭이 생성된다. 그럼 앞으로는 `git abc` 이런 방식으로 해당 별칭만 입력해도 동일한 명령을 실행할 수 있다.
    + 추가한 별칭을 다시 확인하고 싶으면 `git config --list` 을 실행해서 목록의 마지막 쯤에 확인할 수 있다. 별칭을 지우는 방법은 나도 몰라서 탐색기에서 직접 파일을 찾아서 텍스트를 지웠다.

### <b><i>Git</i></b> 의 원격 저장소 생성 및 저장 방법
> 이때까지는 대부분 로컬 저장소인 <b><i>Git</i></b>을 사용하는 기본 방법을 이해했다면, 지금부터 볼 내용은 작업한 파일과 커밋 내역을 원격 저장소인 <b><i>Github</i></b>에 어떻게 올리고 내려받을지에 대한 이야기를 할 것이다.   

#### <b><i>Github</i></b>에 새로운 저장소(Repository) 만들기
- 우선 [깃허브](https://github.com/)에 접속해서 내 이메일과 비밀번호로 로그인한 후에 창 우측 상단에 [ ＋ &#x23F7; ] 메뉴의 [ &#x1F4D2; New repository ]를 선택하면 된다.
![깃허브-메인헤더메뉴](pics/Github_Main_Header_Menu.png)

- 그러면 new repository 창이 나타나면서 repository의 이름과 여러 옵션들을 선택할 수 있다.
![깃허브-새로운레포지토리](pics/Github_Repository_Create_a_new_repository_1.png)
    1. <i>Repository name</i> 입력 란에는 생성할 레포지토리의 이름을 입력하면 된다. '<i>__ is available.</i>'이라고 뜨는 것은 이 이름을 사용할 수 있다는 의미이다. 
    ![](pics/Github_Repository_Create_a_new_repository_repository_name_already_exists.png)
    만일 내 깃허브 계정 내에 같은 이름의 레포지토리가 존재한다면 이렇게 뜰 것이다.
    2. <i>Description</i> 입력 란에는 생성할 레포지토리의 간단한 설명을 적으면 된다. <sub>(optional)</sub>이기 때문에 적지 않고 그대로 두어도 무방하다.
    3. <i>공개(Public)/비공개(Private)</i> 선택 란에는 내가 작업한 내용을 모두가 볼 수 있도록 `Public`으로 두거나, 내가 원하는 몇 사람에게만 보여주기 위해 `Private`으로 선택할 수 있다. (<u>무료 계정인 경우 `Private`에는 인원 수에 제한이 있다.</u>)
- 그 다음 아래 옵션들은 보통 크게 신경쓰지 않아도 되지만, 특정한 언어 소스코드 파일을 저장하거나 협업을 위한 가이드를 제시하기 위해 필요한 옵션이 되기도 한다.
![](pics/Github_Repository_Create_a_new_repository_2.png)
    1. [ ] `Add a README file` 체크 란에 체크하면 레포지토리를 생성할 때 자동으로 <b>README.md</b> 파일을 생성하게 만들 수 있다. 그러나 레포지토리 생성 이후에도 언제든지 간단하게 만들 수 있으니 당장 체크하지 않아도 괜찮다.
    2. 나머지 두 옵션은 고급 옵션에 해당하므로 지금은 건들지 말자. 이제 모든 옵션 세팅을 마쳤으면 `Create Repository` 버튼을 누르면 드디어 레포지토리 하나가 생성된다.

> 이제 다시 <b><i>Git Bash</i></b>로 돌아와서 로컬 저장소와 원격 저장소를 연결해줄 것이다.

#### <b><i>Git</i></b>에서 원격 저장소 등록하기 - `git remote`
1. `git remote add <repo_name> <remote_URL>`
    + 우선 원격 저장소를 로컬 저장소에 지정하는 명령어이다. 처음 원격 저장소를 지정할 때 통상적으로 `add origin` 이라고 쓴다고 한다. 물론 평생 혼자 사용할 저장소이고 이름이 별로 마음에 들지 않는다면, 얼마든지 다른 이름으로 지정해도 된다. 이때 지정한 이름은 <u>원격 브랜치</u>로 명명하기도 한다.
    + 뒤에 URL 주소는 <b><i>Github</i></b>에서 방금 생성한 레포지토리에 들어가서 [ <> Code ] 버튼을 클릭하면 나오는 [ Local ] 탭의 [ HTTPS ] 탭에 띄어 있는 주소를 복사해서 가져오면 된다. (혹은 브라우저 상단에 URL을 복사해도 된다.)
2. `git remote [-v]`
    + 지정한 원격 저장소(remote repository)의 목록을 보고 싶을 때 사용하는 명령어이다. `-v` 옵션을 사용하면 해당 원격 저장소들의 각 주소와 기능(?)을 함께 표시해준다. 
    ![](pics/GitBash_print_git_remote_-v.png) 
    마지막 괄호 안에 있는 `fetch`와 `push`는 앞으로 얘기할 원격 저장소와 로컬 저장소가 주고받는 명령어들이다.
3. `git remote rename <old_repo_name> <new_repo_name>`
    + 이름 그대로 원격 저장소로 지정한 이름을 수정하는 명령어이다. 기존에 있었던 이름과 새로 지을 이름을 띄어쓰기로 구분해 작성하면 된다.
4. `git remote remove <repo_name>`
    + 원격 저장소로 지정한 브랜치를 지우는 명령어이다. 만일 `git remote add`로 원격 브랜치를 여러 개 생성했다면 하나 지우는 게 보통 크게 문제되지 않지만, 원격 브랜치가 하나만 있다면 지우지 않는 걸 추천한다.
 
#### <b><i>Github</i></b>에 나의 로컬 저장소의 커밋 내역을 원격 저장소에 올리기 - `git push`
> 앞으로 `git remote`로 생성한 원격 저장소를 '원격 브랜치(remote branch)'라고 부르겠다. 원격 브랜치를 하나 생성했다면 <b><i>Github</i></b>에 내 커밋 내역을 그대로 올리는 것도 한 번은 해보는 것이 필요하다.

> &#x1F4A1; 여기서 주의할 점은 <b>Git 저장소</b>로 지정된 디렉터리가 C드라이브 혹은 문서, 데스크탑(바탕화면) 등과 같이 <u>너무 포괄적인 위치</u>로 되어있다면 `push`를 하는 것이 굉장히 위험해질 수 있다. 그러므로 `ls -a` 명령어로 탐색했을 때 만일 그런 위치에 `.git/` 폴더가 있는 걸 눈치챘다면 나의 중요한 파일들이 인터넷에 공개되기 전에 그 폴더를 얼른 삭제하고 새로운 하위폴더를 만들어 그 하위 디렉터리에 가서 다시 `git init`을 통해 다시 새로운 저장소를 만들기 바란다. (<b><u>내 PC를 지키기 위해 정말 중요하다.</u></b>)

1. `git push [-u] <remote_branch_name> <local_branch_name>`

#### <b><i>Github</i></b>의 원격 저장소에서 추가된 커밋(변경된 내용)을 내 저장소에 내려받기 `Clone` | `Pull`

### <b>`git status`</b> 명령 결과 분석하기